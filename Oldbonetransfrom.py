...
class SimsVismeaSoundsPanelold(bpy.types.Panel):
    bl_label = "Sims Vismea (Old)"
    bl_idname = "OBJECT_PT_bone_transformold"
    ...
    def poll(cls, context):
        ...
        return preferences.Boners_enum1 == "OPTION2"
...
class BoneTransformAAOperatorold(bpy.types.Operator):
    """Moves the characher mouth to momick the sound of AA"""
    ...
    def execute(self, context):
        ...
        # Rotate the bones
        lower_mouth_area.rotation_euler.x += 0.0872665
        upper_mouth_area.rotation_euler.x -= 0.0872665
        Right_Mouth.location.z -= 0.0132921
        Left_Mouth.location.z -= 0.0132921
        lolip.location.z -= 0.0132921
        if jaw:
            jaw.rotation_quaternion.rotate(Quaternion((1, 0, 0), 0.122173))
